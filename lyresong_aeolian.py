...
Modified to use Aeolian mode (A-B-C-D-E-F-G) instead of Dorian
Aeolian mode suggested by Stefan Hagel https://www.youtube.com/watch?v=FmQoIjz1zjY
"""
            ...
            for line in f:
                ...
                if len(parts) >= 4:
                    ...
                    try:
                        ...
                        lines_data.append((line_num, notes))
                    except:
                        continue
        
        # Create MIDI files
        for line_num, notes in lines_data:
            self._create_line_midi(line_num, notes, output_dir, epic, book_num)
            
            if line_num % 100 == 0:
                print(f"    Generated {line_num} MIDI files...")
        
        print(f"  Generated {len(lines_data)} MIDI files in {output_dir}/")
        
        # Create concatenated file
        if lines_data:
            self.create_concatenated_midi(output_dir, book_num, len(lines_data), epic)
    
    def _create_line_midi(self, line_num: int, notes: List[str], output_dir: str, epic: str = "iliad", book_num: int = 1):
        """Create MIDI file for a single line with exact timing."""
        ...
        for note_str in notes:
            ...
            if "-" in note_name:
                ...
                if note1 in self.note_to_pitch:
                ...
                if note2 in self.note_to_pitch:
        ...
        filename = f"{epic}_{book_num:02d}_{line_num:03d}.mid"
        filepath = os.path.join(output_dir, filename)
        mid.save(filepath)
    
    def create_concatenated_midi(self, midi_dir: str, book_num: int, total_lines: int, epic: str = "iliad"):
        """Create a concatenated MIDI file from all individual line files."""
        concat_mid = mido.MidiFile(ticks_per_beat=self.ticks_per_beat)
        concat_track = mido.MidiTrack()
        concat_mid.tracks.append(concat_track)
        
        # Set tempo once at the beginning
        concat_track.append(mido.MetaMessage('set_tempo', tempo=self.tempo))
        
        print("  Creating concatenated MIDI file...")
        ...
        for line_num in range(1, total_lines + 1):
            ...
            if os.path.exists(filepath):
                # Read the individual MIDI file
                try:
                    mid = mido.MidiFile(filepath)
                    
                    # Copy note events from first track (skip tempo messages)
                    for msg in mid.tracks[0]:
                        if msg.type in ['note_on', 'note_off']:
                            concat_track.append(msg.copy())
                        elif msg.type == 'set_tempo':
                            # Skip tempo messages after the first one
                            continue
                    
                    # Add small pause between lines (1 beat = 480 ticks)
                    concat_track.append(mido.Message('note_off', note=0, velocity=0, time=480))
                    
                except Exception as e:
                    print(f"    Warning: Could not read {filename}: {e}")
        
        # Save concatenated file
        output_path = os.path.join(midi_dir, "..", f"{epic}_{book_num:02d}_concat.mid")
        concat_mid.save(output_path)
        print(f"  Created concatenated MIDI: {os.path.basename(output_path)}")
...
class LyresongPipeline:
    ...
    def __init__(self):
        self.scale = ['A4', 'B4', 'C5', 'D5', 'E5', 'F5', 'G5']
        self.starting_notes = ['D5', 'C5', 'A4', 'D5']  # For "Lyresong" pattern
        self.pitch_values = {
            'A4': 0, 'B4': 1, 'C5': 2, 'D5': 3, 
            'E5': 4, 'F5': 5, 'G5': 6
        }
    ...
    def generate_validated_melody(self, enhanced_file: str):
        ...
        return all_notes, all_note_info, iliad_lines
    ...
    def _strategy_highest_for_accents(self, syllables, words, word_final_acutes):
        ...
        for syl in syllables:
            if syl['accent_flag'] == 3:  # Circumflex
                ...
                line_notes.append(random.choice(['A4', 'B4', 'C5']))
                line_note_info.append({
                    'syllable': syl['text'], 'type': 'regular', 'word_num': syl['word_number'], 
                    'accent_flag': syl['accent_flag'], 'meter': syl['meter']
                })
    ...
    def _strategy_gradual_descent(self, syllables, words, word_final_acutes):
        ...
        note_choices = ['G5', 'F5', 'E5', 'D5', 'C5', 'B4', 'A4']
        base_idx = 0
            ...
            for idx, (syl_idx, syl) in enumerate(word_syls):
                if syl['accent_flag'] == 3:  # Circumflex
                    ...
                    if has_acute:
                        ...
                        word_notes.append(random.choice(['A4', 'B4', 'C5', 'D5', 'E5']))
                    else:
    ...
    def _generate_with_pleasantness_strategy(self, syllables: List[Dict], strategy: int) -> Tuple[List[str], List[Dict]]:
        ...
        if strategy == 0:
            ...
            prev_note = random.choice(['D5', 'C5', 'B4'])
            ...
            tonal_center = 'D5'  # D as tonal center
            scale_degrees = {'A4': -4, 'B4': -3, 'C5': -2, 'D5': 0, 'E5': 2, 'F5': 3, 'G5': 4}
            ...
            for syl in syllables:
                if syl['accent_flag'] == 3:  # Circumflex
                    ...
                    note = random.choice(['D5', 'C5', 'E5', 'B4', 'D5'])  # D5 appears twice
                    line_notes.append(note)
                    line_note_info.append({
                        'syllable': syl['text'], 'type': 'regular', 'word_num': syl['word_number'], 
                        'accent_flag': syl['accent_flag'], 'meter': syl['meter']
            ...
            motif = [random.choice(['C5', 'D5']), random.choice(['B4', 'A4'])]
            motif_idx = 0
            ...
            pentatonic = ['A4', 'C5', 'D5', 'F5', 'G5']  # A minor pentatonic
            
            for syl in syllables:
                if syl['accent_flag'] == 3:  # Circumflex
                    # Choose from upper pentatonic
                    note1 = random.choice(['G5', 'F5'])
                    note2 = 'D5' if note1 == 'F5' else 'F5'
                    line_notes.extend([note1, note2])
                    line_note_info.extend([
                        {'syllable': syl['text'], 'type': 'circumflex_first', 'word_num': syl['word_number'], 'accent_flag': syl['accent_flag'], 'meter': syl['meter']},
                        {'syllable': syl['text'] + '_2', 'type': 'circumflex_second', 'word_num': syl['word_number'], 'accent_flag': syl['accent_flag'], 'meter': syl['meter']}
                    ...
                    note = random.choice(['A4', 'C5', 'D5', 'C5', 'D5'])  # Favor C5 and D5
                    line_notes.append(note)
                    line_note_info.append({
                        'syllable': syl['text'], 'type': 'regular', 'word_num': syl['word_number'], 
                        'accent_flag': syl['accent_flag'], 'meter': syl['meter']
            ...
            wave_pattern = ['B4', 'C5', 'D5', 'E5', 'D5', 'C5']
            wave_idx = 0
                ...
                if syl['accent_flag'] == 3:  # Circumflex
                    ...
                    try:
                        ...
                        if note == current_note:  # If stuck, jump
                            note = random.choice(['B4', 'C5', 'D5', 'E5'])
                    except:
                        note = random.choice(['B4', 'C5', 'D5', 'E5'])
    ...
    def create_tabulature_output(self, notes: List[str], note_info: List[Dict], 
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"# Title: {epic_name} Book {self.book_num} - Lyresong {'-'.join(self.starting_notes)} for scale {','.join(self.scale)}\n")
            f.write(f"# Subtitle: {epic_name} Book {self.book_num} - {valid_lines}/{total_lines} lines passed prosody validation\n")
            f.write(f"# Scale: {' '.join(self.scale)}\n")
            f.write("#\n")
            f.write("# Format: Line#[*] | Notes | Greek Text | Pleasantness\n")
            ...
            for line_idx, line_data in enumerate(iliad_lines):
                ...
                # Write the line
                f.write(f"{line_marker} | {notes_str:<60} | {greek_str:<80} | {pleasantness:.3f}\n")
    ...
    def create_lilypond_from_tabulature(self, tab_file: str, output_file: str, iliad_lines: List[Dict]):
        ...
        with open(tab_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if len(parts) >= 4:
                    line_info = parts[0].strip()
                    notes_str = parts[1].strip()
                    greek_text = parts[2].strip()
                    pleasantness = float(parts[3].strip())
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('#(set-global-staff-size 16)\n\n')
            f.write('\\header {\n')
    ...
    def run_complete_pipeline_with_html(self, html_file: str, book_num: int = 1):
        ...
        notes, note_info, iliad_lines = self.generate_validated_melody(enhanced_file)
        
        # Continue with rest of pipeline...
        self._complete_pipeline_processing(notes, note_info, iliad_lines, book_num, start_time)
    
    def _complete_pipeline_processing(self, notes, note_info, iliad_lines, book_num, start_time):
        ...
        self.create_tabulature_output(notes, note_info, iliad_lines, all_metrics, tab_file)
    ...
    def run_complete_pipeline(self, book_num=1, epic='iliad', run_num=None):
        ...
        if run_num:
            output_base = f"output_aeolian/run_{run_num}/{epic}/book{book_num}"
        else:
            output_base = f"output_aeolian/{epic}/book{book_num}"
        ...
        tab_file = self.create_tabulature_output(notes, note_info, iliad_lines, all_metrics,
        ...
        print(f"  - midi_files/ (folder with {len(iliad_lines)} MIDI files)")
        print(f"  - {epic}_{book_num:02d}_concat.mid (concatenated MIDI)")
...
def process_all_books(num_runs=1):
    ...
    for run in range(1, num_runs + 1):
        ...
        output_dir = f'output_aeolian/run_{run}'
        os.makedirs(output_dir, exist_ok=True)
...
def create_run_summary(results, run_num):
    ...
    output_file = f'output_aeolian/run_{run_num}/SUMMARY.md'
